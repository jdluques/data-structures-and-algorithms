#include "data_structures/linear/lists/doubly.hpp"
...
TEST_CASE("copy constructor creates deep copy", "[copy]") {
    ...
    ds::DoublyLinkedList<int> copy(list);
    copy.push_back(3);
...
TEST_CASE("move constructor", "[move]") {
    ...
    ds::DoublyLinkedList<int> moved(std::move(list));
...
TEST_CASE("sort works correctly", "[sort]") {
    ds::DoublyLinkedList<int> list;
    list.push_back(4);
    list.push_back(2);
    list.push_back(5);
...
TEST_CASE("works with non-trivial types", "[templates]") {
    ds::DoublyLinkedList<std::string> list;
